...
Builds the JSD model based on the training set and the number of bins and
makes prediction based on the model, the significance level and the testing set.
"""

import numpy as np
from scipy.spatial import distance  # it is used to compute the JensenShannon measure
...
class JSD(Detector):
    ...
    def build_model(self, training_dataset):
        t0 = time()
        jsd = JSDdetector()
        jsd.buildModel(training_dataset['Usage'].to_numpy())
        return jsd, time() - t0
...
class JSDmodel:
    ...
    def setKdist(self, P_Xi):
        ...
        for i in range(nWeeks):
            #JSD measure with log_2
            self.K[i] = distance.jensenshannon(P_Xi[i], self.PX, base=2)
...
class JSDdetector:
    ...
    def buildModel(self, train):
        ...
        # Set JensenShannon distribution (nTrainWeeks)
    ...
    def predictConsumption(self, test):
        ...
        for i in range(nTestWeeks):
            #JSD measure with log_2
            Ka[i] = distance.jensenshannon(P_Xa[i], self.model.PX, base=2)
